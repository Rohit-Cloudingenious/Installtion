...
import getChecklistData from '@salesforce/apex/InstallerDetailFormController.getChecklistData';
import getWalkin from '@salesforce/apex/InstallerDetailFormController.getWalkin';
export default class InstallerCfaChecklistClone extends NavigationMixin (LightningElement) {
    ...
    handleSubmit(){
        ...
        try {
            ...
            console.log('this.selectedCons: '+JSON.stringify(this.selectedCons));
            if (!this.hasChecked) {
    ...
    handleYesChange(event) {
        ...
        if (checklistName === 'Walk-In') {
            this.questions = this.questions.map(question => {
                if (question.apiname === questionId) {
                    question.optionWithValue.Yes = event.target.checked;
                    question.optionWithValue.No = !event.target.checked;
                }
                return question;
            });
        }
        ...
        if (checklistName === 'Refrigeration – Evaporator Section for the Cooler') {
            this.questions1 = this.questions1.map(question => {
                if (question.apiname === questionId) {
                    qquestion.optionWithValue.Yes = event.target.checked;
                    question.optionWithValue.No = !event.target.checked;
                }
                return question;
            });
    ...
    handleDisplayChecklist(checklist) {
        this.handleGetData(); // calling data from backend
        //getWalkinRecords({checklistSection: checklist})
        getWalkin({recId: this.recordId, checklistSection: checklist})
        .then(result => {
            if(checklist == 'Walk-In'){
                this.data1 = result;
                this.questions = result;
                this.allQuestion = this.allQuestion.concat(this.questions);
                console.log('Questions1: '+JSON.stringify(this.questions));
...
//Modal JS
    ...
    handleGetData(){
        ...
        getChecklistData({recId:this.recordId})
        .then(result => {
            console.log('Result is' + JSON.stringify(result));
            if(result != null){
               this.response = result;
               /*this.questions = result.map(item => {
                    return {
                        Id: item.Id,
                        SF_ApiName__c: item.SF_ApiName__c,
                        yesChecked: item.Walkin_Question_One__c.includes('Yes'),
                        noChecked: item.Walkin_Question_Two__c.includes('No'),
                        repairsMadeChecked: item.Walkin_Question_One__c.includes('Repairs Made'),
                        naChecked: item.Walkin_Question_Three__c.includes('NA')
                    };
                });
                console.log('Records are: ' + JSON.stringify(this.questions));
               //console.log('Records are '+ this.response);*/
            }
        })
        .catch(error => {
            console.error('Error fetching data:', error);
